...
In this vignette, we show how the simulator can be used to perform simulations related to the [lasso](http://statweb.stanford.edu/~tibs/lasso/lasso.pdf).

# Betting on Sparsity

In [The Elements of Statistical Learning](http://statweb.stanford.edu/~tibs/ElemStatLearn/), the authors describe a "bet on sparsity" principle that suggests that one should,
...
- In the first line, we name the simulation (both in a computer-friendly and human-readable way); this newly created simulation is then piped^[The `simulator` package imports the pipe operator `%>%` from the [magrittr](https://github.com/smbache/magrittr) package.  The pipe passes the output of a function as the first argument of the next function. This means that instead of writing `sim <- simulate_from_model(sim, nsim = 3)` followed by `sim <- run_method(sim, my_method)` followed by `sim <- evaluate(sim, my_metric)`, we can simply write `sim <- sim %>% simulate_from_model(nsim = 3) %>% run_method(my_method) %>% evaluate(my_metric)`.] to `generate_model`, which creates a sequence of sparse linear models where the sparsity level is varied from 1 to 80; 

- this is then piped to `simulate_from_model`, which creates a total of 12 draws from each model (these are created in 4 chunks of size 3, each getting its own random number generator stream so that we will get the same results regardless of whether they are performed in parallel or sequence); 

- the simulation object is then piped to `run_method`, where both the lasso and ridge are performed (in parallel, across 4 cpus) on all 192 instances (192=16*12; 16 models with 12 draws per model); 
