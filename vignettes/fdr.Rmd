...
[Benjamini and Hochberg (1995)](http://www.jstor.org/stable/2346101) design a procedure (for when these p-values are independent) that controls what they call the *false discovery rate* (FDR), which is the expected proportion of the rejected tests that should not have been rejected:
...
Suppose we return to this simulation several days later and wish to double the number of random draws used.  In the above code, we had 200 draws, which were simulated in 4 chunks of 50 draws each.  The simulator allows us to add to a simulation without requiring us to re-run parts of the simulation that have already been run.  
...
The FDR is the average of the false discovery proportion (FDP).  We can look at these raw values (400 for each method-model pair).
...
We see that the procedure becomes more conservative as the dependence increases, but still does control FDR (which was shown for positive dependence in [Benjamini and Yekutieli, 2001](https://projecteuclid.org/euclid.aos/1013699998)). Looking at $\pi_0=1$, we see for negative $\rho$ some indication of anti-conservativeness.
...
Remaking the table (on the basis of 1000 random draws), the anti-conservativeness appears to persist in the $q=0.05$ case.
